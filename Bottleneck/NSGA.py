    ...
    for gen in range(ngen):
        ...
        while len(offspring) < pop_size:
            ...
            if random.random() < crossover_rate:
                try:
                    c1, c2 = crossover(p1, p2, num_blocks)
                except Exception as e:
                    print(f"交叉时出错: {e}")
                    c1, c2 = p1[:], p2[:]
            else:
            ...
            if random.random() < mutation_rate:
                c1 = mutate(c1, num_blocks)
            if random.random() < mutation_rate:
                c2 = mutate(c2, num_blocks)
        ...
        if gen % 10 == 0:
            min_delay = min(f[0] for f in fitness) if fitness and any(f[0] != float('inf') for f in fitness) else float(
                'inf')
            print(f"第 {gen} 代: 前沿大小 {len(fronts[0]) if fronts and fronts[0] else 0}, 最小延迟 {min_delay:.2f}")
...
def crossover(p1, p2, num_blocks):
    """交叉操作 - 单点交叉"""
    # 确保切割点数量相同
    num_cuts = len(p1)
    ...
    # 确保切割点有效
    child1 = sorted(child1)
    child2 = sorted(child2)
    return child1, child2


def mutate(individual, num_blocks):
    """变异操作 - 随机改变一个切割点"""
    num_cuts = len(individual)

    if num_cuts == 0:
        return individual  # 不需要切割点

    # 随机选择一个切割点进行变异
    idx = random.randint(0, num_cuts - 1)

    # 生成新的切割点位置，确保不重复且在有效范围内
    possible_positions = [i for i in range(1, num_blocks) if i not in individual]
